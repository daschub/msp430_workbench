...
void commandHandler(void)
{
    ...
    } else if (strcasecmp("calender", parameter1) == 0){
...
{
    if (strcasecmp("print", parameter2) == 0){
        if (strcasecmp("date", parameter3) == 0){
            calender = getCalender();
            sprintf(transmittedString, "%02d.%02d.%d, %02d:%02d:%02d Uhr \r\n", calender.day, calender.month,
                calender.year, calender.hour, calender.minute, calender.second);
        } else if (strcasecmp("time", parameter3) == 0){
            sprintf(transmittedString, "%02d:%02d:%02d Uhr \r\n", calender.hour, calender.minute, calender.second);
        }
        uart_send(transmittedString);
    } // if print
    else if (strcasecmp("change", parameter2) == 0){
        if (strcasecmp("date", parameter3) == 0){
            uart_send("Gebe Datum im Format DD.MM.YYYY,hh:mm:ss ein\r\n");
            //commmand_changeCalender = 1;
            validateCalender(); // TODO
        }
    } // else if change
    else {
        uart_send("parameter nicht erlaubt\r\n");
    }
}
